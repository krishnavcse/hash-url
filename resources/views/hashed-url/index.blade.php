...
<main class="col bg-faded py-3 flex-grow-1">
			...
			<table id="url-data-table" class="table table-striped table-bordered" style="width:100%">
				...
				<tbody>
					...
					<tr>
						...
						<td class="hashed_url"> <a href="{{ $value->tiny_url }}"> {{ $value->tiny_url }} </a> </td>
						<td class="url">{{ $value->url_click_count }}</td>
						<td class="is_active">{{ $value->is_active ? 'Active' : 'Deactive' }} </td>
	...
	<script>
		$(document).on('click','.update',function()
		{
			...
			$('#hashed_url').val(_this.find('.hashed_url').text());
            ...
            $('.delete_url').click(function(e) {
			    ...
			    $.ajax(
			        ...
			        success: function (response){
			        	...
			        	window.location.href = "{{ url('hashed-urls') }}";
			        }
			    });
			});
